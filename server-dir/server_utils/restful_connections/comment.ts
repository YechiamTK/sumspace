 ...
 * @param mongoose
 */
 export async function newReplyToComment(router: Router, mongoose: Mongoose){
  router.post('/new-reply-comment', async (req: Request, response: Response)=>{
    console.log("entered new-reply-comment");
    const { userId, commentText, summaryId, level, ancestors } = req.body.params;
