...
import { UserBE, ArticleBE, AuthorBE, SummaryBE, TagBE,
  retrieveAuthorSchema, retrieveUserSchema, retrieveSummarySchema, retrieveArticleSchema, retrieveTagSchema, retrieveCommentSchema, CommentBE } from './schemas';
...
export async function fullSummary(router: Router, mongoose: Mongoose){
  router.post('/full-summary', (req: Request, res: Response)=>{
    ...
    summaryModel.findOne({'_id': summaryId})
                .populate('user').populate('comments.comment').populate('article').populate('tags')
                .lean().exec().then((result: LeanDocument<SummaryBE>)=>{
                  console.log(result);
                  res.send(result);
        }).catch((err)=>{
